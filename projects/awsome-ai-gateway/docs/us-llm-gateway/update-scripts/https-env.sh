...
#
#   source https-env.sh mygw.click        # or: HTTPS_DOMAIN in config.env, or $DOMAIN already exported
#   source https-env.sh                   # re-run any time (new shell, after step 1 created zone/cert)
#
...
if [ -z "$DOMAIN" ]; then
  echo "https-env: usage: source https-env.sh <domain>   (e.g. mygw.click)" >&2
else
  export DOMAIN
