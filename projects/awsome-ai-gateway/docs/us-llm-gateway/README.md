...
|---|---|---|---|
| [**US-13**](claude-code/installer/cc-installer-admin-e2e-windows.md) 2026/09 | Claude Code **Windows 설치 파일** — 관리자가 설치 파일 1개를 만들어 배포하면 직원은 실행 후 명령 두 개로 끝난다(지금은 PC 마다 Python·저장소·PATH 를 손으로 맞춘다) | 선택 · Windows 직원 PC 가 있으면 권장 · 게이트웨이는 바뀌지 않는다 | 빌드용 PC 에서 설치 파일 만들기 → 직원 PC 에 설치 → 로그인 1회 |
