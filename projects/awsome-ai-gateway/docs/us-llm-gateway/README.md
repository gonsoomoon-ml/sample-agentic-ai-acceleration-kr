...
- 🔴 **코드는 fork 의 `us/deploy-fixes` 브랜치에서 받습니다** — https://github.com/gonsoomoon-ml/sample-agentic-ai-acceleration-kr/tree/us/deploy-fixes/projects/awsome-ai-gateway (원본 [aws-samples](https://github.com/aws-samples/sample-agentic-ai-acceleration-kr) 에 아직 없는 배포·벤더 픽스 포함. `forked from aws-samples/…` 배너가 정상)
- fork 는 upstream 위로 **리베이스**되므로 커밋 해시가 바뀝니다 → 버전은 해시가 아니라 **`US-NN`** 으로 셉니다.
...
|---|---|---|---|---|---|
| `US-06` 2026/08 | ALB HTTPS — 커스텀 도메인 + ACM | 선택 · **운영이면 강력 권장** | §3-6 시점에 | 도메인 확보 → 전환 → 클라이언트 URL 2개 교체 (30분) | [ops/8-H](ops/8-H-alb-https.md) |
| `US-05` 2026/08 | EKS 1.31 → 1.34 | 필수(지원 만료·비용) | 포함 | 1단계씩 3회 apply + 전 ns 재시작 | [ops/8-E](ops/8-E-eks-upgrade.md) |
| `US-04` 2026/08 | Bedrock·STS 를 VPC Endpoint 로 | 필수(컴플라이언스) | 포함 | 엔드포인트 apply → gateway-proxy 재시작 | [ops/8-N](ops/8-N-vpc-endpoint.md) |
...
**① 저장소 최신화** — 리베이스 브랜치라 `git pull` 이 아니라 아래. `values-*.yaml` 은 이 EC2 유일본이라 백업·복원이 핵심(`values restored OK` 확인).

```bash
cd ~/awsome-ai-gateway && git remote -v            # origin = gonsoomoon-ml/… 이어야 (aws-samples 면 set-url)
...
**② 상태 점검** — 라이브 시스템(DB 행·엔드포인트·이미지·ALB)을 조회해 판정, 구성 변경 없음, 1~2분(일회용 psql 파드).

```bash
cd ~/awsome-ai-gateway/docs/us-llm-gateway/update-scripts && bash status.sh     # 근거 원문: --verbose
```
```
   OK   US-01  최초 설치 (기준선)
