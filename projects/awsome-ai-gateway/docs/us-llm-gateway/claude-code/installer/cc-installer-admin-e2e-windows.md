...
| 모델 지정 | `--model` 만 지정하고 로스터는 비워 둔다 | 허용 모델은 서버(팀·사용자 허용 목록)가 정한다. `--available-models` 를 박으면 모델을 새로 등록할 때마다 PC 마다 `setup` 을 다시 돌려야 한다 |
...
모델 선택 목록까지 PC 에 고정하고 싶을 때만 로스터를 줍니다. 목록 밖의 값은 `setup` 이
거부하므로 오타는 막히지만, 모델을 새로 등록하면 그 PC 들은 `setup` 을 다시 돌려야 합니다.
평소에는 `--model` 만 주고, 허용 범위는 서버의 팀·사용자 허용 목록에 맡깁니다.
