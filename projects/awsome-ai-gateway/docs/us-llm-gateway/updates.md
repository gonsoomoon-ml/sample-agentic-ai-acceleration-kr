...
|---|---|---|---|
| [**US-08**](ops/8-L-admin-ui-login.md) 2026/08 | Admin UI Cognito 로그인 — dev-login 대체 | 선택 · **운영이면 강력 권장** · 신규 포함(별도 적용) | 이미지 재빌드 → `setup-admin-ui-login.sh` → install-eks → `devLoginEnabled=false` |
