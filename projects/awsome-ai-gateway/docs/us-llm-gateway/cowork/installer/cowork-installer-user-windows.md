...
**업데이트는 자동이 아니다.** offline `.msix` 로 깐 앱은 사이드로드(Developer 서명)라 앱 안에
업데이트 안내가 뜨지 않는다. 새 모델이 "이 버전에서는 지원하지 않는다" 는 오류를 내면 대개 이
경우다. **먼저 관리자에게 확인한 뒤**, 위 offline 주소에서 최신 `.msix` 를 받아 같은 방법으로
설치한다.
