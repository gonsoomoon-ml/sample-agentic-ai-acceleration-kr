...
from api_key_helper.sso import (
    check_sso_session,
    create_presigned_sts_request,
    get_device_name,
    get_sso_session_expiry,
)
...
def main(gateway_url: str | None, daemon: bool, check_interval: int, verbose: bool, auth_mode: str) -> None:
    ...
    from api_key_helper.config import apply_baked_env_fallback, load_config
    from gateway_cli_oidc.tls import enable_os_trust_store

    # Seed OIDC/admin env from the build-time baked defaults when unset. A
    # Start-menu MSIX launch of Claude Desktop does not inherit HKCU\Environment,
    # so without this the inferenceCredentialHelper VK refresh would fail in that
    # clean environment. Must run before _detect_mode() (which keys off
    # OIDC_ISSUER_URL/OIDC_CLIENT_ID) and before load_oidc_config_from_env().
    apply_baked_env_fallback()
    ...
    # on stdout's default logger and corrupts the token-only stdout contract that
    # Claude Desktop's inferenceCredentialHelper (and Claude Code's apiKeyHelper) rely on.
